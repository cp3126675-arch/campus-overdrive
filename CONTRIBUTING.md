...
从本次开发工程修复起，标准入口 `npm run dev` 自动执行本地 D1 迁移并启动 Worker 和 Vite；使用与正式榜单相同的接口和规则，但数据库独立，无需 Cloudflare 账号或令牌。

- 使用终端中的 `http://127.0.0.1:3000/` 或 `http://localhost:3000/`。端口被占用时执行 `npm run dev -- --port 3001`；不要直接运行 Vite 命令，游戏需要本地 API 一同启动。
- 本地 `/leaderboard.json` 动态指向当前地址，`/api` 由 Vite 代理到随机端口的回环 Worker。生产配置仍指向正式榜单，不放宽正式跨域白名单。
- 昵称、参榜身份及成绩保存在 `.wrangler/campus-dev/`；关闭终端后再次启动会保留。可用环境变量 `CAMPUS_DEV_STATE_DIR` 指定另一数据目录。不要提交本地数据库、`.dev.vars`、令牌或配置凭据。
- 浏览器身份属于当前地址，`localhost` 与 `127.0.0.1`（以及不同端口）各自独立。若手动删除本地数据库，或者以前连接过自建服务而出现“参榜身份已失效”，请清除**该本地地址**的浏览器站点数据并重新取名，勿清除正式游戏的数据。
- 本地页面内的排行榜是开发测试榜，不会向正式全服榜写成绩。这个入口仅支持本机回环地址，不代表已支持手机局域网联调。可选 `dev:site` 是维护者原入口，不包含本次自动服务流程。

执行 `npm run test:dev` 可验证完整本地链路，包括竞速与生存计分、改名同步和重启持久化；测试使用临时数据库，结束自动清理。更新源码后先停止旧的开发进程，再运行 `npm ci` 和 `npm run dev`。
